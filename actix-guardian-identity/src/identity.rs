...
/// The extractor type to obtain your auth from a request.
///
/// ```
/// use actix_web::*;
...
impl Identity {
    /// Return the claimed auth of the user associated request or `None` if no auth can be
    ...
    /// This method is used to 'forget' the current auth on subsequent requests.
    pub fn forget(&self) {
        if let Some(id) = self.0.extensions_mut().get_mut::<IdentityItem>() {
            id.id = None;
            id.changed = true;
