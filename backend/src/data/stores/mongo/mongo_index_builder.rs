...
/// ```
///
/// let index = MongoIndex::new_with_direction("username", SortOrder::Descending)
...
/// Collection of indexes. Provides function to build data commands.
///
/// [Mongo manual](https://docs.mongodb.com/manual/indexes/)
#[derive(Debug, Clone)]
pub struct Indexes(pub(crate) Vec<MongoIndex>);
...
impl From<Vec<MongoIndex>> for Indexes {
    fn from(indexes: Vec<MongoIndex>) -> Self {
        Self(indexes)
    }
}
...
impl Indexes {
    ...
    pub fn with(mut self, index: MongoIndex) -> Self {
        self.0.push(index);
        self
    }
...
pub async fn sync_indexes(db: &Database, coll_conf: CollectionConfig) -> AppResult<()> {
    let mut indexes = coll_conf.indexes;

    let ret = h_run_command(db, doc! { "listIndexes": coll_conf.collection_name }).await?;
    ...
    if !to_drop.is_empty() {
        ...
        if h_run_command(
            db,
            doc! { "dropIndexes": coll_conf.collection_name, "index": &to_drop },
        )
        .await
        .is_err()
        {
            for index_name in to_drop {
                h_run_command(
                    db,
                    doc! { "dropIndexes": coll_conf.collection_name, "index": index_name },
                )
                .await?;
            }
        }
    ...
    if !indexes.0.is_empty() {
        h_run_command(
            db,
            indexes.create_indexes_command(coll_conf.collection_name),
        )
        .await?;
    }
...
async fn h_run_command(db: &Database, command_doc: Document) -> AppResult<Document> {
    let resp = db
        .run_command(
            command_doc,
...
mod tests {
    ...
    fn create_indexes_command() {
        let index = MongoIndex::new_with_direction("id", SortOrder::Descending)
            .with_key("last_seen")
            .with_option(IndexOption::Background)
            .with_option(IndexOption::Unique);

        let index_2 = MongoIndex::new("last_seen").with_option(IndexOption::ExpireAfterSeconds(60));
